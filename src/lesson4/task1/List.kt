...
fun factorize(n: Int): List<Int> {
    ...
    while (digit > 0 && divider <= Math.round(Math.sqrt(n.toDouble())).toInt()) {
        if (digit % divider == 0) {
...
fun roman(n: Int): String {
    ...
    var i = 0
    var strWithDigits = ""
    while (num > 0 && i in 0 until roman.size) {
        if (num >= arab[i]) {
            num = num - arab[i]
            strWithDigits += roman[i]
        } else i++
    }
...
fun russian(n: Int): String {
    ...
    val sss = listOf("десять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать",
            "шестнадцать", "семнадцать", "восемнадцать", "девятнадцать")
    val tens = listOf("", "десять", "двадцать", "тридцать", "сорок", "пятьдесят", "шестьдесят",
            "семьдесят", "восемьдесят", "девяносто")
    val handreds = listOf("", "сто", "двести", "триста", "четыреста", "пятьсот", "шестьсот",
